  ...
  return (
    <>
      {hardcodedDoctors.map((doctor) => (
        <section
          key={doctor.id}
          className="border border-text_grey/40 rounded-md shadow-lg py-1 px-1 bg-gray-300/20 transition duration-300 ease-in-out shadow-md transform hover:scale-105 hover:shadow-md"
        >
          {loading ? (
            <SkeletonLoading />
          ) : (
